...
| タイトル | ゲーム名・遊び方3行・ベストスコア・TAP TO START | 開始前の説明と入口 |

### タイトル画面が必要な理由

起動直後にいきなり盤面が出ると、プレイヤーは何をすればよいか分からない。とくに本作のルール（半分より多く覆うと獲得、覆いきれなければ重ねてよい、外すと失う）は盤面を見ただけでは推測できない。

タイトル画面では盤面を暗く透かした上に、ゲーム名と遊び方3行、ベストスコア、`TAP TO START` を重ねる。画面のどこをタップしても開始する。

`?seed=` が指定されていればそのシードで開始するため、盤面の共有はタイトル画面を経由しても機能する。
