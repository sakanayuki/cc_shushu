...
|---:|---|---:|---|---|---:|
| 0 | Easy | 1 | 0.15 〜 0.40 | 80 〜 100 | 10 〜 20 |
| 1 | Easy | 1 | 0.15 〜 0.40 | 80 〜 100 | 10 〜 20 |
| 2 | Normal | 1 | 0.40 〜 0.70 | 54 〜 76 | 30 〜 50 |
| 3 | Normal | 1 | 0.40 〜 0.70 | 54 〜 76 | 30 〜 50 |
| 4 | Hard | 1 | 0.70 〜 0.95 | 36 〜 50 | 60 〜 100 |

寸法はすべて**半径**（論理px）である。いずれの難易度でも投げカード1枚で閾値50%に到達できる（[02-rules.md](./02-rules.md) 2.6節）。
...
`MAX_PLACEMENT_ATTEMPTS` の半分（100回）を超えたら、`TARGET_MIN_GAP` を段階的に緩和する（30 → 15 → 0）。これにより、わずかに詰まっているだけの盤面で不必要に配置を諦めることを防ぐ。`C4`（投げカードとの非重複）だけは**絶対に緩和しない**。
